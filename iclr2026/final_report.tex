...
\paragraph{Signed Contribution ($S_i$).}
\begin{equation}
S_i = \frac{1}{N_i} \sum_{x:\, i \in \text{top-}k(x)} \frac{\langle G_i(x) \cdot E_i(x),\; h_{\text{post}}(x) \rangle}{\|h_{\text{post}}(x)\|_2}
\label{eq:si}
\end{equation}
$S_i$ captures directional alignment with the layer output. We originally hypothesized that harmful experts would show negative $S_i$ (opposing the output direction); in practice Expert~2's $S_i$ is small but positive, meaning it is a zero-contributor rather than an active adversary. $C_i$ magnitude is the more discriminative signal.
...
\paragraph{Signed contribution.}
Expert~2's $S_i$ is 0.003--0.55 across layers (other experts: 23--315). The small positive values confirm Expert~2 does not actively oppose the output direction; it simply contributes almost nothing. The $C_i$ magnitude criterion is therefore sufficient to flag it as a prune candidate; the sign of $S_i$ is not required.
